...
#define net_device	AppleIXGbe

#include "ixgbe.h"
#include "ixgbe_dcb_82599.h"
#include "ixgbe_sriov.h"

#include "AppleIXGbe.h"
...
static bool netif_carrier_ok(AppleIXGbe* p)
{
	return p->carrier();
}

static void netif_carrier_off(AppleIXGbe* p)
{
	p->setCarrier(false);
}

static void netif_carrier_on(AppleIXGbe* p)
{
	p->setCarrier(true);
}

static bool netif_queue_stopped(AppleIXGbe* p)
{
	return p->queueStopped();
}

static bool netif_running(AppleIXGbe* p)
{
	return p->running();
}

static void netif_wake_queue(AppleIXGbe* p)
{
	p->startTxQueue();
}

static void netif_tx_wake_all_queues(AppleIXGbe* p)
{
	netif_wake_queue(p);
}


static void netif_tx_start_all_queues(AppleIXGbe* p)
{
	netif_wake_queue(p);
}


static void netif_tx_stop_all_queues(AppleIXGbe* p)
{
	p->stopTxQueue();
}

static void netif_tx_disable(AppleIXGbe* p)
{
	netif_tx_stop_all_queues(p);
}
#define	netif_stop_subqueue(a,b)	netif_tx_stop_all_queues(a)

struct ixgbe_adapter* netdev_priv(AppleIXGbe* p)
{
	return &p->priv_adapter;
}

static void netif_set_gso_max_size(AppleIXGbe* p, UInt32 maxSize)
{
	p->gso = maxSize;
}

static void netif_device_detach(AppleIXGbe* netdev)
{
	netdev->detachNetIf();
}


static sk_buff* netdev_alloc_skb_ip_align(IOEthernetController* netdev, u16 rx_buffer_len)
{
    mbuf_t m = netdev->allocatePacket(rx_buffer_len);
    mbuf_pkthdr_setlen(m, 0);
	sk_buff* skb = (sk_buff*)IOMalloc(sizeof(sk_buff));
	bzero(skb,sizeof(sk_buff));
	skb->m = m;
	return skb;
}
...
static uint32_t _jiffies()
{
	...
	return  (uint32_t)(seconds * 100 + microsecs / 10000); // 10 ms
}
#define	HZ	250
...
#ifdef __APPLE__
		adapter->netdev->scheduleService();
#else
										  ...
										  u64 qmask)
{
	...
	switch (adapter->hw.mac.type) {
		case ixgbe_mac_82598EB:
			mask = (u32)(IXGBE_EIMS_RTX_QUEUE & qmask);
			IXGBE_WRITE_REG(&adapter->hw, IXGBE_EICS, mask);
			break;
		case ixgbe_mac_82599EB:
		case ixgbe_mac_X540:
			mask = (u32)(qmask & 0xFFFFFFFF);
									  ...
									  struct ixgbe_tx_buffer *tx_buffer)
{
	if (tx_buffer->m) {
#ifdef __APPLE__
		ring->netdev->freePacket(tx_buffer->m);
#else
...
{
	u32 tx_done = (u32)ixgbe_get_tx_completed(tx_ring);
	u32 tx_done_old = (u32)tx_ring->tx_stats.tx_done_old;
	u32 tx_pending = (u32)ixgbe_get_tx_pending(tx_ring);
	bool ret = false;
							   ...
							   struct ixgbe_ring *tx_ring)
{
	...
	do {
		...
		/* free the skb */
#ifdef __APPLE__
		tx_ring->netdev->freePacket(tx_buffer->m);
#else
		dev_kfree_skb_any(tx_buffer->skb);
	...
	if (check_for_tx_hang(tx_ring) && ixgbe_check_tx_hang(tx_ring)) {
		...
		e_err(drv, "tx_buffer_info[next_to_clean]\n"
			  "  time_stamp           <%lx>\n",
			  tx_ring->tx_buffer_info[i].time_stamp);
...
#define		CHECKSUM_UNNECESSARY	\
	IONetworkController::kChecksumTCP | CSUM_TCPIPv6 | \
	IONetworkController::kChecksumUDP | CSUM_UDPIPv6
#endif
									...
									struct ixgbe_rx_buffer *bi)
{
	...
	if (likely(!page)) {
#ifdef __APPLE__
		page = IOBufferMemoryDescriptor::inTaskWithOptions( kernel_task, kIODirectionInOut | kIOMemoryPhysicallyContiguous,
							PAGE_SIZE * (1 << ixgbe_rx_pg_order(rx_ring)),
							PAGE_SIZE );
#else
...
#ifdef __APPLE__
	struct ixgbe_adapter *adapter = q_vector->adapter;
	adapter->netdev->receive(skb->m,skb->vid,skb->ip_summed);
	IOFree(skb,sizeof(sk_buff));
#else
	struct ixgbe_adapter *adapter = q_vector->adapter;
...
#ifdef __APPLE__
		rx_ring->netdev->freePacket(skb->m);
		IOFree(skb,sizeof(sk_buff));
		// add error report....
#else
		dev_kfree_skb_any(skb);
							  ...
							  struct sk_buff *skb)
{
    ...
    mbuf_copyback(skb->m, mbuf_pkthdr_len(skb->m), size,
				  va,
				  MBUF_WAITOK);
    return true;
#else /* __APPLE__ */
							   ...
							   int budget)
{
	...
	do {
		...
		total_rx_bytes += (u32)mbuf_pkthdr_len(skb->m);
 ...
 **/
#ifdef __APPLE__
int ixgbe_poll(struct ixgbe_q_vector *q_vector, int budget)
#else
 ...
 **/
#ifdef __APPLE__
void AppleIXGbe::interruptHandler(OSObject * target, IOInterruptEventSource* src, int count )
{
	AppleIXGbe * me = (AppleIXGbe *) target;
	me->handleInterrupt(count);
}
void AppleIXGbe::handleInterrupt( int count )
{
	struct ixgbe_adapter *adapter = &priv_adapter;
	struct ixgbe_hw *hw = &adapter->hw;
							...
							struct ixgbe_ring *ring)
{
	...
	IXGBE_WRITE_REG(hw, IXGBE_TDBAL(reg_idx), (u32)(tdba & DMA_BIT_MASK(32)));
	IXGBE_WRITE_REG(hw, IXGBE_TDBAH(reg_idx), (u32)(tdba >> 32));
	IXGBE_WRITE_REG(hw, IXGBE_TDLEN(reg_idx),
					(u32)(ring->count * sizeof(union ixgbe_adv_tx_desc)));
							 ...
							 struct ixgbe_ring *ring)
{
	...
	IXGBE_WRITE_REG(hw, IXGBE_RDBAL(reg_idx), (u32)(rdba & DMA_BIT_MASK(32)));
	IXGBE_WRITE_REG(hw, IXGBE_RDBAH(reg_idx), (u32)(rdba >> 32));
	IXGBE_WRITE_REG(hw, IXGBE_RDLEN(reg_idx),
					(u32)(ring->count * sizeof(union ixgbe_adv_rx_desc)));
...
#ifdef __APPLE__
	u8* addr = *mc_addr_ptr;
	*mc_addr_ptr += sizeof(IOEthernetAddress);
	return addr;
#else
 ...
 **/
#ifdef __APPLE__
IOReturn AppleIXGbe::setMulticastList(IOEthernetAddress * addrs, UInt32 count)
{
	ixgbe_adapter *adapter = &priv_adapter;
	struct ixgbe_hw *hw = &adapter->hw;
...
void ixgbe_reset(struct ixgbe_adapter *adapter)
{
	...
	memcpy(old_addr, &adapter->mac_table[0].addr, kIOEthernetAddressSize);
	ixgbe_flush_sw_mac_table(adapter);
	memcpy(&adapter->mac_table[0].addr, old_addr, kIOEthernetAddressSize);
...
#ifdef __APPLE__
			{
			rx_buffer->page->complete();
			rx_buffer->page->release();
			}
#else
...
static int __devinit ixgbe_sw_init(struct ixgbe_adapter *adapter)
{
	...
	adapter->mac_table = (ixgbe_mac_addr*)kzalloc(sizeof(struct ixgbe_mac_addr) *
...
int ixgbe_setup_rx_resources(struct ixgbe_ring *rx_ring)
{
	...
	size = (u32)sizeof(struct ixgbe_rx_buffer) * rx_ring->count;
...
#ifdef __APPLE__
	rx_ring->rx_buffer_info = (ixgbe_rx_buffer*)IOMallocPageable(size,PAGE_SIZE);
#else
	...
	/* Round up to nearest 4K */
	rx_ring->size = rx_ring->count * (u32)sizeof(union ixgbe_adv_rx_desc);
...
err:
#ifdef __APPLE__
	IOFreePageable(rx_ring->rx_buffer_info,size);
#else
...
#ifdef __APPLE__
	size_t size = sizeof(struct ixgbe_tx_buffer) * tx_ring->count;
	IOFreePageable(tx_ring->tx_buffer_info, size);
#else
	vfree(tx_ring->tx_buffer_info);
#endif
	tx_ring->tx_buffer_info = NULL;
...
#ifdef __APPLE__
	size_t size = sizeof(struct ixgbe_rx_buffer) * rx_ring->count;
	IOFreePageable(rx_ring->rx_buffer_info, size);
#else
	vfree(rx_ring->rx_buffer_info);
#endif
 ...
 **/
#ifdef __APPLE__
IOReturn AppleIXGbe::setMaxPacketSize (UInt32 max_frame)
{
	IOLog("AppleIXGbe::setMaxPacketSize(%d)\n",(int)max_frame);
	struct ixgbe_adapter *adapter = &priv_adapter;
	UInt32 new_mtu = max_frame  - (ETH_HLEN + ETH_FCS_LEN);
...
IOReturn AppleIXGbe::getMaxPacketSize (UInt32 *maxSize) const {
	if (maxSize)
		*maxSize = IXGBE_MAX_JUMBO_FRAME_SIZE;
	
	return kIOReturnSuccess;
}

IOReturn AppleIXGbe::getMinPacketSize (UInt32 *minSize) const {
	if(minSize)
		*minSize = 68 + (ETH_HLEN + ETH_FCS_LEN);
...
#ifdef __APPLE__
static int __ixgbe_shutdown(struct ixgbe_adapter *adapter, bool *enable_wake)
#else
...
{
#ifdef __APPLE__
	IONetworkStats* net_stats = adapter->netdev->stats();
#else
...
#ifdef __APPLE__
	net_stats->inputErrors = (UInt32)(hwstats->crcerrs + hwstats->rlec);
#else
 ...
 **/
#ifdef __APPLE__
void AppleIXGbe::timerHandler(OSObject* target, IOTimerEventSource * src)
{
	AppleIXGbe * me = (AppleIXGbe *) target;
	me->handleTimer();
}
		
void AppleIXGbe::handleTimer()
{
	struct ixgbe_adapter *adapter = &priv_adapter;
...
#endif
	/* Reset the timer */
	timerSource->setTimeoutMS((UInt32)next_event_offset);
 ...
 **/
#ifdef __APPLE__
void AppleIXGbe::serviceHandler(OSObject* target, IOTimerEventSource * src)
{
	AppleIXGbe * me = (AppleIXGbe *) target;
	me->handleService();
}
		
void AppleIXGbe::handleService()
{
	struct ixgbe_adapter *adapter = &priv_adapter;
					 ...
					 u8 *hdr_len)
{
...
#ifdef	__APPLE__
	u_int32_t dataLen = (u_int32_t)mbuf_pkthdr_len(skb);
	struct tcphdr* tcph;
	int ip_len;
	...
	*hdr_len = ETHER_HDR_LEN + ip_len + l4len;
	...
	mss_l4len_idx |= mssValue << IXGBE_ADVTXD_MSS_SHIFT;
	
	/* VLAN MACLEN IPLEN */
	vlan_macip_lens = ip_len;
	vlan_macip_lens |= ETHER_HDR_LEN << IXGBE_ADVTXD_MACLEN_SHIFT;
...
#define     DEMAND_MASK (DEMAND_IPv6|DEMAND_IPv4)
	...
	if(checksumDemanded == 0){
		if (!(first->tx_flags & IXGBE_TX_FLAGS_CC))
			return;
	} else {
		int  ip_hlen;
		u8* packet;
		...
		if(checksumDemanded & IONetworkController::kChecksumTCP){
			...
			mss_l4len_idx = (tcph->th_off << 2) << IXGBE_ADVTXD_L4LEN_SHIFT;
...
static void ixgbe_tx_map(struct ixgbe_ring *tx_ring,
						 struct ixgbe_tx_buffer *first,
						 const u8 hdr_len,
						 UInt32 count, IOPhysicalSegment* vec)

{
	...
	ixgbe_tx_olinfo_status(tx_desc, tx_flags, (unsigned int)mbuf_pkthdr_len(first->m) - hdr_len);
	
	dma = vec[0].location;
	size = (unsigned int)vec[0].length;
	...
	for (k=1;;k++) {
		...
		dma = vec[k].location;
		size = (unsigned int)vec[k].length;
...
#ifdef __APPLE__
UInt32 AppleIXGbe::ixgbe_xmit_frame_ring(mbuf_t m,
								...
								struct ixgbe_ring *tx_ring)
{
	...
	count = txMbufCursor->getPhysicalSegmentsWithCoalesce(m, vec, MAX_SKB_FRAGS);
	...
	first->m = m;
	first->bytecount = (u32)mbuf_pkthdr_len(m);
	first->gso_segs = 1;
	
	/* if we have a HW VLAN tag being added default to the HW one */
	UInt32 vlan;
	if(getVlanTagDemand(m,&vlan)){
		//IOLog("vlan(out) = %d\n",(int)vlan);
		tx_flags |= vlan << IXGBE_TX_FLAGS_VLAN_SHIFT;
...
xmit_fcoe:
#endif /* IXGBE_FCOE */
	ixgbe_tx_map(tx_ring, first, hdr_len, count, vec);
...
#ifdef __APPLE__
/*
 * ixgbe_xmit_frame is replaced by outputPacket.
 */
UInt32 AppleIXGbe::outputPacket(mbuf_t m, void * param)
{
	struct ixgbe_adapter *adapter = &priv_adapter;
	struct ixgbe_ring *tx_ring;
	...
	return ixgbe_xmit_frame_ring(m, adapter, tx_ring);
}
 ...
 **/
#ifdef __APPLE__
IOReturn AppleIXGbe::setHardwareAddress(const IOEthernetAddress * addr)
{
	struct ixgbe_adapter *adapter = &priv_adapter;
	struct ixgbe_hw *hw = &adapter->hw;
	...
	ixgbe_add_mac_filter(adapter, hw->mac.addr, VMDQ_P(0));
	return kIOReturnSuccess;
}
#else
 ...
 **/
#ifdef __APPLE__
bool AppleIXGbe::start(IOService * provider)
{
	...
	if (IOEthernetController::start(provider) == false)
		return false;
	...
	memcpy(hw->mac.addr, hw->mac.perm_addr, IXGBE_ETH_LENGTH_OF_ADDRESS);
	...
	interruptSource = IOInterruptEventSource::interruptEventSource(this,&AppleIXGbe::interruptHandler,provider);
	serviceSource = IOTimerEventSource::timerEventSource(this, &AppleIXGbe::serviceHandler );
	getWorkLoop()->addEventSource(serviceSource);
	
	timerSource = IOTimerEventSource::timerEventSource(this, &AppleIXGbe::timerHandler );
	getWorkLoop()->addEventSource(timerSource);
	...
	if (ixgbe_wol_supported(adapter, hw->device_id, hw->subsystem_device_id))
	...
	e_dev_info("(PCI Express:%s:%s) %02x:%02x:%02x:%02x:%02x:%02x\n",
			   ...
			   (hw->bus.width == ixgbe_bus_width_pcie_x8 ? "Width x8" :
				...
				"Unknown"), hw->mac.addr[0], hw->mac.addr[1],
	...
	if (err)
		memcpy(part_str, "Unknown", IXGBE_PBANUM_LENGTH);
 ...
 **/
#ifdef __APPLE__
void AppleIXGbe::stop(IOService * provider)
{
	struct ixgbe_adapter *adapter = &priv_adapter;
	...
	if(workLoop){
		if (serviceSource) {
			serviceSource->cancelTimeout();
			workLoop->removeEventSource(serviceSource);
			RELEASE(serviceSource);
		}
		if (timerSource) {
...
IOReturn AppleIXGbe::enable(IONetworkInterface * netif)
{
	if(!enabledForNetif){
		pdev->open(this);
...
void AppleIXGbe::getPacketBufferConstraints(IOPacketBufferConstraints * constraints) const
{
...
IOOutputQueue * AppleIXGbe::createOutputQueue()
{
	return IOGatedOutputQueue::withTarget(this, getWorkLoop());
}

bool AppleIXGbe::configureInterface(IONetworkInterface * interface)
{
	
	IONetworkData * data = NULL;
	
	if (IOEthernetController::configureInterface(interface) == false) {
...
bool AppleIXGbe::createWorkLoop()
{
	workLoop = IOWorkLoop::workLoop();
	return (workLoop !=  NULL);
}
		
IOWorkLoop * AppleIXGbe::getWorkLoop() const
{
	return workLoop;
}
		

IOReturn AppleIXGbe::disable(IONetworkInterface * netif)
{
	if(enabledForNetif){
...
const OSString * AppleIXGbe::newVendorString() const
{
	return OSString::withCString("Intel");
}

const OSString * AppleIXGbe::newModelString() const
{
	static struct  {
		UInt16 id;
		const char* name;
	...
	{
		{ IXGBE_DEV_ID_82598, "82598"},
		{ IXGBE_DEV_ID_82598AF_DUAL_PORT, "82598AF Dual"},
		{ IXGBE_DEV_ID_82598AF_SINGLE_PORT, "82598AF Single"},
		{ IXGBE_DEV_ID_82598AT, "82598AT"},
		{ IXGBE_DEV_ID_82598AT2, "82598AT2"},
		{ IXGBE_DEV_ID_82598EB_CX4, "82598EB CX4"},
		{ IXGBE_DEV_ID_82598_CX4_DUAL_PORT, "82598 CX4 Dual"},
		{ IXGBE_DEV_ID_82598_DA_DUAL_PORT, "82598DA Dual"},
		{ IXGBE_DEV_ID_82598_SR_DUAL_PORT_EM, "82598SR Dual EM"},
		{ IXGBE_DEV_ID_82598EB_XF_LR, "82598EB XF LR"},
		{ IXGBE_DEV_ID_82598EB_SFP_LOM, "82598EB SFPLOM"},
		{ IXGBE_DEV_ID_82598_BX, "82598BX"},
		{ IXGBE_DEV_ID_82599_KX4, "82599KX4"},
		{ IXGBE_DEV_ID_82599_XAUI_LOM, "82599 XAUI LOM"},
		{ IXGBE_DEV_ID_82599_KR, "82599KR"},
		{ IXGBE_DEV_ID_82599_SFP, "82599SFP"},
		{ IXGBE_DEV_ID_82599_SFP_EM, "82599SFP EM"},
		{ IXGBE_DEV_ID_82599_KX4_MEZZ, "82599KX4 MEZZ"},
		{ IXGBE_DEV_ID_82599_CX4, "82599CX4"},
		{ IXGBE_DEV_ID_82599_BACKPLANE_FCOE, "82599 BACKPLANE FCOE"},
		{ IXGBE_DEV_ID_82599_SFP_FCOE, "82599_SFP_FCOE"},
		{ IXGBE_DEV_ID_82599_T3_LOM, "82599T3 LOM"},
		{ IXGBE_DEV_ID_82599_COMBO_BACKPLANE, "82599COMBO BACKPLANE"},
		{ IXGBE_DEV_ID_X540T, "X540T"},
		{ IXGBE_DEV_ID_82599_SFP_SF2, "82599SFP SF2"},
		{ IXGBE_DEV_ID_82599_LS, "82599LS"},
		{ IXGBE_DEV_ID_82599EN_SFP, "82599EN SFP"},
	};
	int k;
	u16 device = priv_adapter.pdev->configRead16(kIOPCIConfigDeviceID);
	for( k = 0; k < sizeof(decieModelNames)/sizeof(decieModelNames[0]); k++){
		if(device == decieModelNames[k].id )
...
IOReturn AppleIXGbe::registerWithPolicyMaker ( IOService * policyMaker )
{
...
IOReturn AppleIXGbe::setPowerState( unsigned long powerStateOrdinal,
								 IOService *policyMaker )
{
	if (powerState == powerStateOrdinal)
		return IOPMAckImplied;
	powerState = (UInt32)powerStateOrdinal;
...
IOReturn AppleIXGbe::getHardwareAddress(IOEthernetAddress * addr)
{
	memcpy(addr->bytes, priv_adapter.hw.mac.addr, kIOEthernetAddressSize);
	return kIOReturnSuccess;
}
	
IOReturn AppleIXGbe::setPromiscuousMode(bool active)
{
	if(active)
		flags |= IFF_PROMISC;
	else
	...
	ixgbe_set_rx_mode(this);
	return kIOReturnSuccess;
}
	
IOReturn AppleIXGbe::setMulticastMode(bool active)
{
	if(active)
		flags |= IFF_ALLMULTI;
	else
...
IOReturn AppleIXGbe::setWakeOnMagicPacket(bool active)
{
	ixgbe_adapter *adapter = &priv_adapter;
	if(active){
		if (adapter->wol == IXGBE_WUFC_MAG)
			return kIOReturnUnsupported;
		adapter->hw.wol_enabled = TRUE;
	} else {
		adapter->hw.wol_enabled = FALSE;
	}
	return kIOReturnSuccess;
}
	
IOReturn AppleIXGbe::getPacketFilters(const OSSymbol * group, UInt32 * filters) const {
...
UInt32 AppleIXGbe::getFeatures() const {
#ifdef NETIF_F_TSO
#ifdef NETIF_F_TSO6
...
IOReturn AppleIXGbe::getChecksumSupport(UInt32 *checksumMask, UInt32 checksumFamily, bool isOutput)
{
	*checksumMask = 0;
...
IOReturn AppleIXGbe::selectMedium(const IONetworkMedium * medium)
{
	IOReturn rc = kIOReturnSuccess;
	if(medium == NULL)
		medium = mediumTable[MEDIUM_INDEX_AUTO];
	UInt64 speed = medium->getSpeed();
	...
	if ((hw->phy.media_type == ixgbe_media_type_copper) ||
		...
		u32 advertised = 0;
		if(speed == 10000L * MBit)
			advertised = IXGBE_LINK_SPEED_10GB_FULL;
		else if(speed == 1000L * MBit)
			advertised = IXGBE_LINK_SPEED_1GB_FULL;
		else if(speed == 100L * MBit)
			advertised = IXGBE_LINK_SPEED_100_FULL;
		else /* auto negotation */
			advertised = IXGBE_LINK_SPEED_10GB_FULL | IXGBE_LINK_SPEED_1GB_FULL;
		/* this sets the link speed and restarts auto-neg */
		hw->mac.autotry_restart = true;
		s32 err = hw->mac.ops.setup_link(hw, advertised, true);
		if (err) {
...
bool AppleIXGbe::addNetworkMedium(UInt32 type, UInt64 bps, UInt32 index)
{
...
bool AppleIXGbe::init(OSDictionary *properties)
{
	
	if (IOEthernetController::init(properties) == false)
		return false;
	...
	addNetworkMedium(kIOMediumEthernet10GBaseT | kIOMediumOptionFullDuplex,
					 10000L * MBit, MEDIUM_INDEX_10000FD);
	addNetworkMedium(kIOMediumEthernet1000BaseT | kIOMediumOptionFullDuplex,
					 1000L * MBit, MEDIUM_INDEX_1000FD);
	addNetworkMedium(kIOMediumEthernet100BaseTX | kIOMediumOptionFullDuplex,
					 100L * MBit, MEDIUM_INDEX_100FD);

	return true;
}

void AppleIXGbe::free()
{
	RELEASE(workLoop);
	RELEASE(csrPCIAddress);
	RELEASE(pdev);
...
/////
void AppleIXGbe::detachNetIf()
{
	detach(netif);
	RELEASE(netif);
}

void AppleIXGbe::receive(mbuf_t m, UInt32 vlanTag, UInt32 csumFlag)
{
	if(csumFlag)
		setChecksumResult(m, kChecksumFamilyTCPIP, csumFlag, csumFlag );
	if(vlanTag)
		setVlanTag(m, vlanTag);
	netif->inputPacket(m, (UInt32)mbuf_pkthdr_len(m));
}
		
bool AppleIXGbe::carrier()
{
	return (preLinkStatus & kIONetworkLinkActive) != 0;
	
}
	
void AppleIXGbe::setCarrier(bool stat)
{
	if(stat){
		preLinkStatus = kIONetworkLinkValid | kIONetworkLinkActive;
		UInt64 speed = 10000L * MBit;
...
void AppleIXGbe::startTxQueue()
{
	IOLog("AppleIGB::startTxQueue()\n");
	txMbufCursor = IOMbufNaturalMemoryCursor::withSpecification(mtu + ETH_HLEN + ETH_FCS_LEN, MAX_SKB_FRAGS);
	transmitQueue->start();
	bQueueStopped = false;
}
	
void AppleIXGbe::stopTxQueue()
{
