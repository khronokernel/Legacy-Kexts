...
#define synchronize_irq(a)
#define ASSERT_RTNL()
#define WARN_ON(a)


#define netif_tx_disable(netdev)	(netdev->stopQueue())
#define netif_stop_queue(netdev)	(netdev->stopQueue())
#define	netif_start_queue(netdev)	(netdev->startQueue())
#define netif_wake_queue(netdev)	(netdev->wakeQueue())
#define netif_queue_stopped(netdev)	(netdev->queueStopped())
#define netif_carrier_on(netdev)	(netdev->carrierOn())
#define netif_carrier_off(netdev)	(netdev->carrierOff())
#define netif_carrier_ok(netdev)	(netdev->carrierOK())

#define	netdev_priv(netdev)	(netdev->priv())
#define pci_set_master(pdev)	(pdev->setBusMasterEnable(true))
#define netif_running(netdev)	(netdev->running())

#define test_and_set_bit(b,a)	(set_bit(b, a),0)
#define rtnl_lock()
#define rtnl_unlock()

#define SET_MODULE_OWNER(a)
#define SET_NETDEV_DEV(a, b)
#define pci_set_drvdata(a,b)
#define	pci_release_regions(pdev)
#define pci_disable_device(pdev)
#define free_netdev(netdev)


#define skb_trim(skb,n)	mbuf_setlen(skb,n)
#define	netdev_alloc_skb_ip_align(netdev, bufsz)	(netdev->allocatePacket(bufsz))

static inline u8 skb_transport_offset(mbuf_t skb)
{
	// from FreeBSD
	struct ip * ip = (struct ip *)((u8*)mbuf_data(skb) + ETHER_HDR_LEN);
	u32 ip_hlen = ip->ip_hl << 2;
	return ip_hlen + ETHER_HDR_LEN;
}
...
#ifdef __APPLE__
static int e1000_probe(IOPCIDevice* pdev, AppleIntelE1000 *netdev);
static void __devexit e1000_remove(AppleIntelE1000 *netdev);
#else
static int e1000_probe(IOPCIDevice* pdev, const struct pci_device_id *ent);
static void __devexit e1000_remove(IOPCIDevice* pdev);
#endif
static int e1000_alloc_rings(struct e1000_adapter *adapter);
static int e1000_sw_init(struct e1000_adapter *adapter);
static int e1000_open(AppleIntelE1000* netdev);
static int e1000_close(AppleIntelE1000* netdev);
...
static void e1000_clean_rx_ring(struct e1000_adapter *adapter,
                                struct e1000_rx_ring *rx_ring);
static void e1000_set_multi(AppleIntelE1000* netdev);
...
static int e1000_xmit_frame(struct sk_buff *skb, AppleIntelE1000* netdev);
static void e1000_phy_read_status(struct e1000_adapter *adapter);
static struct net_device_stats * e1000_get_stats(AppleIntelE1000* netdev);
static int e1000_change_mtu(AppleIntelE1000* netdev, int new_mtu);
static int e1000_set_mac(AppleIntelE1000* netdev, void *p);
...
static void e1000_alloc_rx_buffers(struct e1000_adapter *adapter,
                                   struct e1000_rx_ring *rx_ring,
                                   int cleaned_count);
static int e1000_ioctl(AppleIntelE1000* netdev, struct ifreq *ifr, int cmd);
#ifdef SIOCGMIIPHY
static int e1000_mii_ioctl(AppleIntelE1000* netdev, struct ifreq *ifr,
                           int cmd);
#endif
static void e1000_enter_82542_rst(struct e1000_adapter *adapter);
static void e1000_leave_82542_rst(struct e1000_adapter *adapter);
static void e1000_tx_timeout(AppleIntelE1000* dev);
...
static void e1000_vlan_rx_register(AppleIntelE1000* netdev,
                                   struct vlan_group *grp);
static void e1000_vlan_rx_add_vid(AppleIntelE1000* netdev, u16 vid);
static void e1000_vlan_rx_kill_vid(AppleIntelE1000* netdev, u16 vid);
static void e1000_restore_vlan(struct e1000_adapter *adapter);
#endif

#if defined(CONFIG_PM) || defined(USE_REBOOT_NOTIFIER)
static int e1000_suspend(IOPCIDevice* pdev, pm_message_t state);
#endif
#ifdef CONFIG_PM
static int e1000_resume(IOPCIDevice* pdev);
#endif
#ifndef USE_REBOOT_NOTIFIER
static void e1000_shutdown(IOPCIDevice* pdev);
#else
...
static void e1000_netpoll (AppleIntelE1000* netdev);
#endif
...
#ifdef HAVE_PCI_ERS
static pci_ers_result_t e1000_io_error_detected(IOPCIDevice* pdev,
                     pci_channel_state_t state);
static pci_ers_result_t e1000_io_slot_reset(IOPCIDevice* pdev);
static void e1000_io_resume(IOPCIDevice* pdev);
...
{
	AppleIntelE1000* netdev = adapter->netdev;
 ...
 **/
#ifdef __APPLE__
static int __devinit e1000_probe(IOPCIDevice* pdev,
...
#endif
{
...
#ifdef __APPLE__
	pci_using_dac = 0;
	pdev->configWrite16( kIOPCIConfigCommand, pdev->configRead16( kIOPCIConfigCommand )|kIOPCICommandMemWrInvalidate );
	...
	err = -EIO;
#ifdef __APPLE__
	adapter->hw.hw_addr = (u8*)netdev->csrPCIAddress->getVirtualAddress();
#else	/* __APPLE__*/
	...
	for (i = 0; i < 6; i++)
		IOLog("%2.2x%c", adapter->hw.mac.addr[i], i == 5 ? '\n' : ':');
...
err_eeprom:
	...
	IOFree(adapter->rx_ring,sizeof(struct e1000_rx_ring));
err_sw_init:
#ifdef __APPLE__
	RELEASE(netdev->csrPCIAddress);
#else
...
#endif
{
	//AppleIntelE1000* netdev = pci_get_drvdata(pdev);
...
{
	adapter->tx_ring = (e1000_tx_ring*)IOMalloc(sizeof(struct e1000_tx_ring));
	if (!adapter->tx_ring)
		return -ENOMEM;
	bzero(adapter->tx_ring,sizeof(struct e1000_tx_ring));

	adapter->rx_ring = (e1000_rx_ring*)(e1000_tx_ring*)IOMalloc(sizeof(struct e1000_rx_ring));
	if (!adapter->rx_ring) {
		IOFree(adapter->tx_ring,sizeof(struct e1000_tx_ring));
		return -ENOMEM;
	}
...
static int e1000_set_mac(AppleIntelE1000* netdev, void *p)
{
	...
	memcpy(adapter->hw.mac.addr, p, ETH_ALEN);
#else
 ...
 * the phy */
#ifdef __APPLE__
void AppleIntelE1000::e1000_update_phy_info_task()
{
	struct e1000_adapter *adapter = this->priv();
	rtnl_lock();
	e1000_get_phy_info(&adapter->hw);
	rtnl_unlock();
}
 ...
 **/
#ifdef __APPLE__
void AppleIntelE1000::e1000_82547_tx_fifo_stall_task()
{
	struct e1000_adapter *adapter = this->priv();
	AppleIntelE1000* netdev = this;
#else
static void e1000_82547_tx_fifo_stall_task(struct work_struct *work)
{
	struct e1000_adapter *adapter = container_of(work,
...
#ifdef __APPLE__
			netdev->fifostallSource->setTimeoutMS(10);
#else
...
#ifdef __APPLE__
void AppleIntelE1000::e1000_watchdog_task()
{
	struct e1000_adapter *adapter = this->priv();
	AppleIntelE1000* netdev = this;
	//E1000_DBG("e1000_watchdog_task: prevLink = %d",(int)preLinkStatus);
#else
static void e1000_watchdog_task(struct work_struct *work)
{
	struct e1000_adapter *adapter = container_of(work,
...
#ifdef __APPLE__
				netdev->phyinfoSource->setTimeoutMS(2*HZ);
#else
...
link_up:
...
#ifdef	__APPLE__
	netStats->inputErrors = adapter->net_stats.rx_errors;
	netStats->outputErrors = adapter->net_stats.tx_errors;
#endif
	...
	if (!test_bit(__E1000_DOWN, &adapter->state))
#ifdef __APPLE__
		watchdogSource->setTimeoutMS(2 * HZ);
#else
					 ...
					 unsigned int* tx_flags )
{
    ...
    u_int32_t value;
    if(mbuf_get_tso_requested(skb, &request, &value))
		return false;

    if(request & (MBUF_TSO_IPV4|MBUF_TSO_IPV6)){
		mss = value;
		...
		if (mss) {
			...
			unsigned int max_per_txd = (1 << max_txd_pwr);
							...
							unsigned int* tx_flags)
{
	...
	UInt32 csum_offset = 0;

	netdev->getChecksumDemand(skb, IONetworkController::kChecksumFamilyTCPIP, &checksumDemanded);
#if	0
	if(	checksumDemanded & IONetworkController::kChecksumIP ){
		struct ip* ip = (struct ip *)((u8*)mbuf_data(skb) + ETHER_HDR_LEN);
		context_desc->lower_setup.ip_fields.ipcss = ETHER_HDR_LEN;
		context_desc->lower_setup.ip_fields.ipcse = cpu_to_le16(ETHER_HDR_LEN + (ip->ip_hl << 2));
		context_desc->lower_setup.ip_fields.ipcso = ETHER_HDR_LEN + offsetof(struct ip, ip_sum);
...
#ifdef	__APPLE__
	context_desc->upper_setup.tcp_fields.tucso = css + csum_offset;
#else
...
#ifdef __APPLE__
						IOPhysicalSegment* tx_segments,
#endif
                        unsigned int mss)
{
	struct e1000_buffer *buffer_info;
#ifdef __APPLE__
	unsigned int count, i, f;
	...
	unsigned int len = mbuf_pkthdr_len(skb);
...
#ifdef __APPLE__
	u32 skb_fifo_len = mbuf_pkthdr_len(skb) + E1000_FIFO_HDR;
#else
...
static int __e1000_maybe_stop_tx(AppleIntelE1000* netdev,
...
static int e1000_maybe_stop_tx(AppleIntelE1000* netdev,
                               struct e1000_tx_ring *tx_ring, int size)
{
	if (likely(E1000_DESC_UNUSED(tx_ring) >= ((size) * tx_ring->step)))
		return 0;
 ...
 * @netdev: network interface device structure
 **/
static void e1000_tx_timeout(AppleIntelE1000* netdev)
{
#ifdef __APPLE__
	netdev->e1000_reset_task();
#else
...
#ifdef __APPLE__
void AppleIntelE1000::e1000_reset_task()
{
	struct e1000_adapter *adapter = this->priv();
 ...
 **/
#ifdef __APPLE__
int AppleIntelE1000::e1000_intr()
{
	AppleIntelE1000* netdev = this;
#else
static irqreturn_t e1000_intr(int irq, void *data)
{
	AppleIntelE1000* netdev = (AppleIntelE1000*)data;
#endif
	struct e1000_adapter *adapter = netdev_priv(netdev);
	struct e1000_hw *hw = &adapter->hw;
#ifdef __APPLE__
	...
	if (unlikely(icr & (E1000_ICR_RXSEQ | E1000_ICR_LSC))) {
		...
		if (!test_bit(__E1000_DOWN, &adapter->state))
#ifdef __APPLE__
			adapter->netdev->watchdogSource->setTimeoutMS(10);
#else
...
#ifdef __APPLE__
	netStats->inputPackets += adapter->total_rx_packets;
	netStats->outputPackets += adapter->total_tx_packets;
	if(adapter->total_rx_packets)
...
{
#ifdef __APPLE__
	adapter->netdev->receive(skb,le16_to_cpu(vlan) & E1000_RXD_SPC_VLAN_MASK);
#else	/* __APPLE__ */
...
#ifdef __APPLE__
			u8* mbuf_data = (u8*)buffer_info->pool->getBytesNoCopy();
			u8 last_byte = *(mbuf_data + length - 1);
#else
			u8 last_byte = *(skb->data + length - 1);
#endif
			if (TBI_ACCEPT(&adapter->hw, status,
				...
				/* recycle */
#ifdef __APPLE__
				E1000_DBG("E1000_RXD_ERR_FRAME_ERR_MASK");
#else
				buffer_info->skb = skb;
#endif
				goto next_desc;
...
#ifdef	__APPLE__
		skb = netdev->allocatePacket(length);
		mbuf_copyback(skb, 0, length, buffer_info->pool->getBytesNoCopy(),MBUF_WAITOK);
                                   ...
                                   int cleaned_count)
{
	...
	while (cleaned_count--) {
		if(buffer_info->pool){
			...
			buffer_info->pool = IOBufferMemoryDescriptor::inTaskWithPhysicalMask(kernel_task,kIODirectionInOut | kIOMemoryPhysicallyContiguous, 
				bufsz,
				0xFFFFFFFFUL - (msize-1));	// within 4GB, msize boundary to avoid 64k crossing
			if(unlikely(buffer_info->pool == NULL)){
				/* Better luck next round */
				adapter->alloc_rx_buff_failed++;
				break;
			}
		...
		if(buffer_info->dma == 0){
			buffer_info->pool->prepare();
			buffer_info->dma = buffer_info->pool->getPhysicalAddress();
		}
...
#ifdef __APPLE__
static int __e1000_shutdown(AppleIntelE1000* netdev, bool *enable_wake)
{
#else
static int __e1000_shutdown(IOPCIDevice* pdev, bool *enable_wake)
{
	AppleIntelE1000* netdev = pci_get_drvdata(pdev);
#endif
	struct e1000_adapter *adapter = netdev_priv(netdev);
...
UInt32 AppleIntelE1000::getFeatures() const
{
...
void AppleIntelE1000::free()
{
	...
	RELEASE(phyinfoSource);
	RELEASE(fifostallSource);
...
bool AppleIntelE1000::start(IOService* provider)
{
	...
	do {
		...
		if (!interruptSource ||
			(workLoop->addEventSource(interruptSource) != kIOReturnSuccess)) {
			break;
		}
...
void AppleIntelE1000::initTimers()
{
	fifostallSource = IOTimerEventSource::timerEventSource(this, &timeoutHandler );
	workLoop->addEventSource(fifostallSource);

	watchdogSource = IOTimerEventSource::timerEventSource(this, &timeoutHandler );
	workLoop->addEventSource(watchdogSource);

	phyinfoSource = IOTimerEventSource::timerEventSource(this, &timeoutHandler );
	workLoop->addEventSource(phyinfoSource);

}

void AppleIntelE1000::delTimers()
{
	if (watchdogSource) {
	...
	if (phyinfoSource) {
		phyinfoSource->cancelTimeout();		// Stop the timer event source.
		workLoop->removeEventSource(phyinfoSource);
		RELEASE(phyinfoSource);
	}
	if (fifostallSource) {
		fifostallSource->cancelTimeout();		// Stop the timer event source.
		workLoop->removeEventSource(fifostallSource);
		RELEASE(fifostallSource);
	}
}
...
UInt32 AppleIntelE1000::outputPacket(mbuf_t skb, void * param)
{
	...
	int count = 0;
	int tso;
	IOPhysicalSegment tx_segments[MAX_SKB_FRAGS];
	 ...
	 */
	if (len < 17) {
		mbuf_adjustlen(skb,17);
		mbuf_pkthdr_setlen(skb, 17);
		bzero((u8*)mbuf_data(skb)+len,17-len);
	}
...
#ifdef MAX_SKB_FRAGS
	nr_frags = adapter->netdev->txMbufCursor->getPhysicalSegmentsWithCoalesce(skb, &tx_segments[0], MAX_SKB_FRAGS);

	for (int f = 0; f < nr_frags; f++)
		count += TXD_USE_COUNT(tx_segments[f].length,
				       max_txd_pwr);
	if (adapter->pcix_82544)
		count += nr_frags;
	...
	tso = e1000_tso(adapter, tx_ring, skb, &tx_flags);
	if (tso < 0) {
		freePacket(skb);
		return kIOReturnOutputSuccess;
	}
	...
	if (likely(tso)) {
		...
		e1000_tx_csum(adapter, tx_ring, skb, &tx_flags);
	}

	e1000_tx_queue(adapter, tx_ring, tx_flags,
	               e1000_tx_map(adapter, tx_ring, skb, first,
	                            max_per_txd, nr_frags, tx_segments, mss));

	trans_start = jiffies;

	/* Make sure there is space in the ring for the next send. */
	e1000_maybe_stop_tx(this, tx_ring, MAX_SKB_FRAGS + 2);

	return kIOReturnOutputSuccess;
}

void AppleIntelE1000::receive( mbuf_t skb, UInt32 vlan )
{
#ifdef NETIF_F_HW_VLAN_TX
	if ((unlikely(adapter->vlgrp && (status & E1000_RXD_STAT_VP))))
		vlan_gro_receive(&adapter->rx_ring->napi, adapter->vlgrp,
...
#else
	if(vlan){
		E1000_DBG("AppleIntelE1000::receive vlan = %x", (unsigned int)vlan);
		setVlanTag(skb,vlan);
	}
...
IOReturn AppleIntelE1000::selectMedium(const IONetworkMedium * medium)
{
	...
	u16 speed_duplex = SPEED_1000;
	if(medium->getSpeed() == 100 * MBit){
		speed_duplex = SPEED_100;
	} else if(medium->getSpeed() == 10 * MBit){
		speed_duplex = SPEED_10;
	}
	if(medium->getType() & kIOMediumOptionHalfDuplex){
		speed_duplex |= DUPLEX_HALF;
	} else {
		speed_duplex |= DUPLEX_FULL;
	}
...
void AppleIntelE1000::startQueue()
{
	if(transmitQueue && !txqActive){
		transmitQueue->start();
		txqActive = true;
	}
}
...
void AppleIntelE1000::wakeQueue()
{
	startQueue();
	transmitQueue->service();
}

bool AppleIntelE1000::queueStopped()
{
	return (transmitQueue == NULL || !txqActive);
}

void AppleIntelE1000::carrierOn()
{
	if(enabledForNetif){
...
void AppleIntelE1000::carrierOff()
{
...
IOReturn AppleIntelE1000::setHardwareAddress(const IOEthernetAddress * addr)
{
	...
	e1000_set_mac(this, (void*)(addr->bytes));
...
void AppleIntelE1000::timeoutHandler(OSObject * target, IOTimerEventSource * src)
{
	...
	AppleIntelE1000* me = (AppleIntelE1000*) target;
	if(src == me->watchdogSource)
		me->e1000_watchdog_task();
	else if(src == me->phyinfoSource)
		me->e1000_update_phy_info_task();
	else if(src == me->fifostallSource)
		me->e1000_82547_tx_fifo_stall_task();
...
extern "C" {
	void e1000_writeRegIO(e1000_hw* hw, u32 reg, u32 offset){
//		outl(reg, ((a)->io_base));
//		outl(offset, ((a)->io_base + 4));
		struct e1000_adapter *adapter = (struct e1000_adapter *)(hw->back);
		adapter->pdev->ioWrite32(offset, reg);
	}
}